{{--Librerías útiles (Bootstrap, fontAwesome, cs-Selects, fullCalendar....)--}}
<link rel="stylesheet" href="{{asset('css/bootstrap/bootstrap.min.css')}}" type="text/css">
<link rel="stylesheet" href="{{asset('css/bootstrap/bootstrap-grid.min.css')}}" type="text/css">
<link rel="stylesheet" href="{{asset('css/bootstrap/bootstrap-reboot.min.css')}}" type="text/css">
<link rel="stylesheet" href="{{asset('css/animate.css')}}" type="text/css">
<link rel="stylesheet" href="{{asset('css/transition.css')}}" type="text/css">
<link rel="stylesheet" href="{{asset('css/fontawesome.min.css')}}" type="text/css">
<link rel="stylesheet" href="{{asset('css/fa-solid.min.css')}}" type="text/css">
<link rel="stylesheet" href="{{asset('css/cs-select.css')}}" type="text/css">
<link rel="stylesheet" href="{{asset('css/cs-skin-slide.css')}}" type="text/css">
<link rel="stylesheet" href="{{asset('css/cs-skin-elastic.css')}}" type="text/css">
<link rel="stylesheet" href="{{asset('css/cs-skin-rotate.css')}}" type="text/css">
<link rel="stylesheet" href="{{asset('css/cs-skin-underline.css')}}" type="text/css">
<link rel="stylesheet" href="{{asset('css/fullcalendar.css')}}" type="text/css">

{{--Css propios--}}
<link rel="stylesheet" href="{{asset('assets/css/main-herramienta.css')}}" type="text/css">

